	...
	)
{
	...
	if(!SC.isPageGenerator()&&!SC.ENVIRONMENT.startLoggedIn&&document.location.href.indexOf("/productrange-all")>-1&&SC.ENVIRONMENT.SCTouchpoint=="shopping"){
	//alert("Redirect to Login "+SC.ENVIRONMENT.startLoggedIn);
      document.location.href=SC.ENVIRONMENT.siteSettings.touchpoints.login+"&l=4&redirect=productrange-all";
...
var profile = ProfileModel.getInstance()
      ...
      if(!SC.isPageGenerator()&&!SC.ENVIRONMENT.startLoggedIn&&url=="productrange-all"&&SC.ENVIRONMENT.SCTouchpoint=="shopping"){
      document.location.href=SC.ENVIRONMENT.siteSettings.touchpoints.login+"&l=3&redirect="+encodeURIComponent(Backbone.history.fragment);
throw("Redirect to Login");
      }else{
			var context = {
				path: path
