...
define('MenuTree.View', [
	'SC.Configuration',
	'Backbone.CompositeView', 'Backbone.CollectionView', 'MenuTree.Node.View', 'Singleton', 'Profile.Model', 'menu_tree.tpl', 'Backbone', 'underscore', 'jQuery'
], function(
	Configuration,
	BackboneCompositeView, BackboneCollectionView, MenuTreeNodeView, Singleton, ProfileModel, menu_tree_tpl
	...
	return Backbone.View.extend({
		...
		*/
		...
		menuClick: function(e, synthetic) {
			...
			, this_expandable = target.closest('[data-type="menu-tree-node-expandable"]'), all_expandables = this.$('[data-type="menu-tree-node-expandable"]');
		...
		updateSidebar: function(label) {
			...
			target_expandable.find('[data-action="expander"]:first').trigger('click', {
				synthetic: true
			});
		...
		getFixedMenuItems: function() {
			...
			_.each(menu_items_with_parent, function(menu_item) {
				...
				if (parent) {
					if (!_.findWhere(parent.children, {
							id: menu_item.id
						})) {
