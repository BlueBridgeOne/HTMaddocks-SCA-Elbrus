...
define(
	'Account.Model', [
		'SC.Model', 'Application', 'Models.Init', 'Profile.Model', 'LiveOrder.Model', 'Address.Model', 'CreditCard.Model', 'SiteSettings.Model', 'underscore'
	],
	function(
		...
		return SCModel.extend({
			...
			register: function(user_data) {
				...
				nlapiLogExecution("DEBUG", "Register", "isGuest:" + ModelsInit.customer.isGuest() + " data:" + JSON.stringify(user_data));
				...
				if (ModelsInit.customer.isGuest()) {
					...
					var custObj = ModelsInit.session.registerCustomer({
						firstname: user_data.firstname,
						lastname: user_data.lastname,
						companyname: user_data.company,
						email: user_data.email,
					...
					try {
						user_data.addressee = user_data.company || (user_data.firstname + " " + user_data.lastname);
						var newcustomer = nlapiGetWebContainer().getShoppingSession().getCustomer();
						newcustomer.addAddress(user_data);
					} catch (err2) {
						throw new Error('Unable to add address. ' + err2);
					}
				}
