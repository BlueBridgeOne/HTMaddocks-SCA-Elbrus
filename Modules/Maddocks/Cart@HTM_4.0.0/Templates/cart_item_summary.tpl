...
<div class="cart-item-summary-item-list-actionable-qty">
	<form action="#" class="cart-item-summary-item-list-actionable-qty-form" data-action="update-quantity" data-validation="control-group">
		...
		<label for="quantity-{{lineId}}" data-validation="control">
			{{#if backOrderItem}}
						<div class="cart-item-summary-back-order">{{translate '<span class="back-order-items">$(0) item(s)</span> will be placed on back order' itemsOnBackOrder}}</div>
					{{/if}}
			{{#if showQuantity}}
			...
			{{else}}
				<div class="cart-item-summary-item-list-actionable-container-qty">
					...
					<div class="cart-item-summary-item-list-actionable-input-qty">
							...
							<input type="number" data-type="cart-item-quantity-input" name="quantity" id="quantity-{{lineId}}" class="cart-item-summary-quantity-value quantity-{{lineId}}" value="{{line.quantity}}" min="{{minimumQuantity}}"/>
