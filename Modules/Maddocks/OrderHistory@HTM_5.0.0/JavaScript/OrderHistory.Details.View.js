	...
	)
{
	...
	,	trackEventReorder: function (line)
		...
		//TODO REMOVE THIS. This code MUST be located in a generic place and only briefly called from the item sub action View.
		//@method addToCart
...
//BB1 Fix for origin being null
var detailedName,originName=_.findWhere(Configuration.get('transactionRecordOriginMapping'), { origin: this.model.get('origin') || null });
if(originName){
	originName=originName.name||"Order";
	detailedName=originName.detailedName||"Order";
}else{
	originName="Order";
	detailedName="Order";
}
