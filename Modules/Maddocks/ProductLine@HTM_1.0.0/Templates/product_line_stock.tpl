	...
	{{else}}
		{{#if showOutOfStockMessage}}
			<p class="product-line-stock-msg-out">
				<i class="fa fa-times" aria-hidden="true"></i><span class="product-line-stock-msg-out-text"> {{stockInfo.outOfStockMessage}}</span>
			</p>
		{{/if}}
		{{#if showInStockMessage}}
			<p class="product-line-stock-msg-in">
				<i class="fa fa-check-circle" aria-hidden="true"></i> {{stockInfo.inStockMessage}}
			</p>
		{{/if}}
	{{/if}}
</div>
