...
define(
	'HandlebarsExtras', [
		'SC.Configuration', 'Utils'

		, 'Handlebars', 'Backbone', 'underscore', 'jQuery'
	],
	function(
		Configuration, Utils

		, Handlebars, Backbone, _, jQuery
	) {
		'use strict';

		Handlebars.registerHelper('equals', function(A, B, options) //BB1 GT
			{
				if (A == B) {
...
Handlebars.registerHelper('notequals', function(A, B, options) //BB1 GT
			{
				if (A != B) {
					return options.fn(this);
				} else {
