	...
	,	_showContent: function _showContent (view, dont_scroll)
		{
			...
			//document's title
			document.title = (view.title || '')+" : Maddocks UK Tradesite";
	...
	,	addModalListeners: function (view, current_view)
		{
			...
			view.$containerModal.on('hidden.bs.modal', function ()
			{
				...
				document.title = (self.currentView && self.currentView.getTitle() || '')+" : Maddocks UK Tradesite";
			});
	...
	,	showInModal: function (view, options)
		{
			...
			if (!view.hasRenderedInModal)
			{
				...
				,	pageHeader: (view.page_header || view.title || '')+" : Maddocks UK Tradesite"
				});
