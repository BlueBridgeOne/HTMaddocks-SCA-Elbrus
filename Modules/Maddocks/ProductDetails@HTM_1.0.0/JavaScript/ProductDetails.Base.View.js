	...
	)
{
	...
	var ProductDetailsBaseView = Backbone.View.extend(
		...
		,	initialize: function initialize (options)
			{
			if(!SC.isPageGenerator()&&!SC.ENVIRONMENT.startLoggedIn&&SC.ENVIRONMENT.SCTouchpoint=="shopping"){
      document.location.href=SC.ENVIRONMENT.siteSettings.touchpoints.login+"&l=4&redirect="+encodeURIComponent(Backbone.history.fragment);
      ...
      , is_loged_in = profile.get('isLoggedIn') === 'T'  && profile.get('isGuest') === 'F',awaitingApproval = profile.awaitingApproval();
      ...
      , is_loged_in = profile.get('isLoggedIn') === 'T'  && profile.get('isGuest') === 'F',awaitingApproval = profile.awaitingApproval();
			...
			isLoggedIn:is_loged_in,
				...
				,	isItemProperlyConfigured: item_model.isProperlyConfigured()
					...
					//TODO REMOVE THIS, this must be implemented as a plugin/extension of this view or any other inner view!!!
					//@property {Boolean} isPriceEnabled
				,	isPriceEnabled: !ProfileModel.getInstance().hidePrices()

					// Custom Fields
				,	newItem: item_model.get('_newItem')
				,	clearanceItem: item_model.get('_clearanceItem')
				,	offerItem: item_model.get('_offerItem')
				,	specItem: item_model.get('_specItem')
				,	specialOrderMessage: item_model.get('_specialOrderMessage')
				,	clearanceQty: item_model.get('_stock')
				,	hidePrice: item_model.get('_hidePrice')
				,	hidePriceMessage: item_model.get('_hidePriceMessage')
				,	discontinuedItem: item_model.get('_discontinuedItem')
				,	isCurrentItemPurchasable: this.model.getItem().get('_isPurchasable')
				,	itemSubstitute1: item_model.get('_substitute1')
				,	itemSubstitute2: item_model.get('_substitute2')
				};
