...
*/
define('Item.KeyMapping', ['underscore', 'Utils', 'UrlHelper'], function(
	_, Utils
) {
	'use strict';
		...
		return {
			...
			_name: function(item) {
				...
				if (parent.get('internalid')) {
					...
					return child_name || parent.get('featureddescription') || parent.get('salesdescription') || parent.get('description') || parent.get('displayname') || item_id;
				}

				// Otherwise return its own name
				return item.get('featureddescription') || item.get('salesdescription') || item.get('description') || item.get('displayname') || item_id;
			}

			// @property {String} _pageTitle Page Title of the PDP
			,
			_pageTitle: ['pagetitle', 'salesdescription', 'storedisplayname2', 'displayname']

			// @property {String}  _pageHeader h1 of the PDP and also the title of the modal
			,
			_pageHeader: ['featureddescription', 'salesdescription', 'description', 'storedisplayname2', 'displayname']
			...
			_breadcrumb: function(item) {
				...
				breadcrumb.push({
					href: item.get('_url'),
					text: item.get('_sku') + ' ' + item.get('_productType') || item.get('_name')
				});
			...
			,
			_priceDetails: function(item) { //BB1 GT Remove first line of price schedule.
				var details = item.get('onlinecustomerprice_detail');
				
				if (details && details.priceschedule && details.priceschedule.length > 1) {
					details.priceschedule.shift();
					details.priceschedule[0].minimumquantity = 0;
				}
				return details;
			}
			...
			// @property {Number} _comparePriceAgainst Returns the default price of the passed in item assuming av minimum quantity. This method check if quantity pricing is configured.
			,
			...
			_isReturnable: function(item) {
				...
				return type === 'InvtPart' || type === 'NonInvtPart' || type === 'Kit' || type === 'Assembly';
			}
			...
			,
			_specialOrderMessage: function(){
				return _('Special Order Item').translate();
			}

			,
			_backOrderMessage: function(){
				return _('This item will be placed on back order').translate();
			}
			...
			,	_itemWeight: 'weight'
			,	_eanCode: 'upccode'
			,	_productClassification: 'custitem_genuine_compatible'
			,	_boxQuantity: 'custitemboxquantity'
			,	_palletQuantity: 'custitempalletquantity'
			,	_brandManufacturer: 'custitem_brand_manufacturer'
			,	_commercialReference: 'custitem_commercial_reference'
			,	_itemDimensions: 'custitem_item_dimensions'
			,	_packQuantity: 'custitem_packquantity'

			//	PDP Fields
			,	_fittingInformation: 'custitem_fitting_information'
			,	_otherInformation: 'custitem_other_information'
			,	_genuineCompatible: 'custitem_gen_orig_comp_description'
			,	_manufacturerInformation: 'custitem_brand_manu_specific_statement'
			,	_consumablesDescription: 'custitem_consumables_description'
			,	_bulkOrdering: 'custitem_box_pallet_qty_description'
			,	_relatedSubstitutes: 'custitem_related_item_sub_description'
			,	_threeSixtyDegreeView: 'custitem_360degreeview'
			,	_videoContent: 'custitem_videocontent'
			,	_hidePrice: 'dontshowprice'
			,	_hidePriceMessage: 'nopricemessage'
			,	_discontinuedItem: 'custitem_discontinued'
			,	_productType: 'custitem_producttype'
			,	_documentationHTML: 'custitem_sca_documentation_html'
			,	_substitute1: 'custitem_substitute1'
			,	_substitute2: 'custitem_substitute2'

			//	New, Offer, Clearance image icons
			,	_newItem: 'custitem_newitem'
			,	_offerItem: 'onspecial'
			,	_clearanceItem: 'custitem_clearanceline'

			//	Special Order Items
			,	_specItem: 'custitem_specialorder'
